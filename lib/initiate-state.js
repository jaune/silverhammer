...
function initiateStateSessionAccountFromSession(req, res, next) {
  ...
  req.app.services.mongodb.collection('accounts').findOne({ uuid: account_uuid }, function (error, account) {
    ...
    req.app.services.mongodb.collection('lobbies').find(query).toArray(function (error, result) {
      if (error) {
        return next(error);
      }

      req.initialState.session.account.lobbies = result.map(function (lobby) {
        return lobby.uuid;
      });
      ...
      result.forEach(function (lobby) {
        req.initialState.collections.lobby[lobby.uuid] = lobby;
      });
...
function initiateStateCollectionPassports(req, res, next) {
  ...
  req.initialState.collections.passport = passports;
  return next();
}
...
module.exports = {
  ...
  initiateStateCollectionPassports: initiateStateCollectionPassports,
